...
class PaperTradingEngine:
    ...
    def _init_db(self) -> None:
        with self._connect() as conn:
            conn.executescript(
                ...
                CREATE TABLE IF NOT EXISTS position_strategy_state (
                    account_id TEXT NOT NULL,
                    symbol TEXT NOT NULL,
                    strategy TEXT NOT NULL,
                    rsi_reduce_done INTEGER NOT NULL DEFAULT 0,
                    ma5_reduce_done INTEGER NOT NULL DEFAULT 0,
                    ma10_reduce_done INTEGER NOT NULL DEFAULT 0,
                    updated_at TEXT NOT NULL,
                    PRIMARY KEY (account_id, symbol, strategy)
                );
                """
            )
    ...
    def get_position_strategy_state(self, account_id: str, symbol: str, strategy: str) -> Dict:
        symbol = self._normalize_symbol(symbol)
        with self._lock, self._connect() as conn:
            row = conn.execute(
                """
                SELECT rsi_reduce_done, ma5_reduce_done, ma10_reduce_done, updated_at
                FROM position_strategy_state
                WHERE account_id = ? AND symbol = ? AND strategy = ?
                """,
            ...
            return {
                "rsi_reduce_done": bool(row["rsi_reduce_done"]),
                "ma5_reduce_done": bool(row["ma5_reduce_done"]),
                "ma10_reduce_done": bool(row["ma10_reduce_done"]),
                "updated_at": row["updated_at"],
            }

    def reset_position_strategy_state(self, account_id: str, symbol: str, strategy: Optional[str] = None) -> None:
        symbol = self._normalize_symbol(symbol)
        with self._lock, self._connect() as conn:
            if strategy:
                conn.execute(
                    "DELETE FROM position_strategy_state WHERE account_id = ? AND symbol = ? AND strategy = ?",
                    (account_id, symbol, strategy),
                )
            else:
                conn.execute(
                    "DELETE FROM position_strategy_state WHERE account_id = ? AND symbol = ?",
                    (account_id, symbol),
                )

    def mark_position_strategy_reduce(self, account_id: str, symbol: str, strategy: str, reason: str) -> Dict:
        symbol = self._normalize_symbol(symbol)
        if reason == "break_ma20":
            self.reset_position_strategy_state(account_id, symbol, strategy)
            return self.get_position_strategy_state(account_id, symbol, strategy)

        flag_map = {
            "rsi_overbought_above_ma5": "rsi_reduce_done",
            "break_ma5": "ma5_reduce_done",
            "break_ma10": "ma10_reduce_done",
        }
        column = flag_map.get(reason)
        if not column:
            raise ValueError(f"unsupported reduce reason: {reason}")

        ts = now_ts()
        with self._lock, self._connect() as conn:
            row = conn.execute(
                """
                SELECT rsi_reduce_done, ma5_reduce_done, ma10_reduce_done
                FROM position_strategy_state
                WHERE account_id = ? AND symbol = ? AND strategy = ?
                """,
                (account_id, symbol, strategy),
            ...
            if row:
                values = {
                    "rsi_reduce_done": int(row["rsi_reduce_done"]),
                    "ma5_reduce_done": int(row["ma5_reduce_done"]),
                    "ma10_reduce_done": int(row["ma10_reduce_done"]),
                }
                values[column] = 1
                conn.execute(
                    f"""
                    UPDATE position_strategy_state
                    SET rsi_reduce_done = ?, ma5_reduce_done = ?, ma10_reduce_done = ?, updated_at = ?
                    WHERE account_id = ? AND symbol = ? AND strategy = ?
                    """,
                    (
                        values["rsi_reduce_done"],
                        values["ma5_reduce_done"],
                        values["ma10_reduce_done"],
                        ts,
                        account_id,
                        symbol,
                        strategy,
                ...
                conn.execute(
                    """
                    INSERT INTO position_strategy_state(
                        account_id, symbol, strategy,
                        rsi_reduce_done, ma5_reduce_done, ma10_reduce_done, updated_at
                    ) VALUES(?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        account_id,
    ...
    def _fill_order(self, conn: sqlite3.Connection, order: sqlite3.Row, fill_price: float, market_date: str) -> None:
        ...
        if order["side"] == "buy":
            ...
            conn.execute(
                "DELETE FROM position_strategy_state WHERE account_id = ? AND symbol = ?",
                (order["account_id"], self._normalize_symbol(order["symbol"])),
            )
        else:
        ...
        if order["side"] == "sell":
            remaining_qty = self._get_total_qty(conn, order["account_id"], order["symbol"])
            if remaining_qty <= 0:
                conn.execute(
                    "DELETE FROM position_strategy_state WHERE account_id = ? AND symbol = ?",
                    (order["account_id"], self._normalize_symbol(order["symbol"])),
