...
def test_latest_history_snapshot_uses_only_closed_today_bars() -> None:
    ...
    with patch("simulated_trading.market_data.get_price", side_effect=[day_df, minute_df]), patch(
        "simulated_trading.market_data.datetime"
    ) as dt_mock:
...
def test_get_quote_recomputes_timestamp_and_change_pct_from_qt() -> None:
    ...
    with patch.object(
    ...
    ) as latest_mock, patch("simulated_trading.market_data._parse_tencent_quote") as parse_mock:
        ...
        with patch("simulated_trading.market_data.datetime") as dt_mock:
            dt_mock.now.return_value = datetime(2026, 6, 17, 10, 37, 0)
