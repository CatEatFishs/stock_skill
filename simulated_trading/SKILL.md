...
python3 "$SKILL_DIR/scripts/simulated_trading_ctl.py" status
python3 "$SKILL_DIR/scripts/simulated_trading_ctl.py" stop
```

在 macOS 上，如需持续自启动，可安装 launchd：

```bash
python3 "$SKILL_DIR/scripts/simulated_trading_ctl.py" install-launchd
```
...
```bash
python3 "$SKILL_DIR/scripts/simulated_trading_cli.py" create-account alpha --cash 500000
python3 "$SKILL_DIR/scripts/simulated_trading_cli.py" list-accounts
python3 "$SKILL_DIR/scripts/simulated_trading_cli.py" show-default-account
python3 "$SKILL_DIR/scripts/simulated_trading_cli.py" set-default-account alpha
python3 "$SKILL_DIR/scripts/simulated_trading_cli.py" reset-account alpha --cash 300000
python3 "$SKILL_DIR/scripts/simulated_trading_cli.py" add-cash alpha 50000 --note 入金
python3 "$SKILL_DIR/scripts/simulated_trading_cli.py" deduct-cash alpha 10000 --note 出金
python3 "$SKILL_DIR/scripts/simulated_trading_cli.py" buy alpha 600519 100 --market
python3 "$SKILL_DIR/scripts/simulated_trading_cli.py" sell alpha 600519 100 --price 1450
python3 "$SKILL_DIR/scripts/simulated_trading_cli.py" orders alpha
python3 "$SKILL_DIR/scripts/simulated_trading_cli.py" positions alpha
python3 "$SKILL_DIR/scripts/simulated_trading_cli.py" show-account alpha
python3 "$SKILL_DIR/scripts/simulated_trading_cli.py" trades alpha
python3 "$SKILL_DIR/scripts/simulated_trading_cli.py" cancel <order_id>
python3 "$SKILL_DIR/scripts/simulated_trading_cli.py" process-orders
python3 "$SKILL_DIR/scripts/simulated_trading_cli.py" run-snapshots
python3 "$SKILL_DIR/scripts/simulated_trading_cli.py" backtest 600519 --strategy sma_cross --start 2025-01-01 --end 2026-03-31 --cash 200000
```
...
- `scripts/simulated_trading_service.py`: 启动 HTTP 服务
- `scripts/simulated_trading_cli.py`: CLI
- `scripts/simulated_trading_ctl.py`: 启停、状态、launchd 安装
- `scripts/simulated_trading_runtime.py`: 运行时默认目录、端口、路径
- `scripts/simulated_trading/`: 账户、撮合、估值、数据适配

## 策略减仓状态（与 trend_strategy 联动）

SQLite 表 `position_strategy_state` 记录持仓分档减仓是否已触发（每种档位仅一次）：

- 主键：`(account_id, symbol, strategy)`
- 字段：`rsi_reduce_done`、`ma5_reduce_done`、`ma10_reduce_done`
- 买入成交或持仓清零时自动清除该标的记录
- 由 `trend_strategy/scripts/daily_decisions.py --paper-account` 读写
